...
namespace Aiminfomatics.Widgets
{
    ...
    public class NewsletterSubscriptionWidgetController : Controller
    {
        ...
        [Route("NewsletterSubscriptionWidgetController/Subscribe")]
        public ActionResult Subscribe(NewsletterSubscriptionSubscribeModel model)
        {
           
            if (!ValidationHelper.IsEmail(model.Email))
            {
                ModelState.AddModelError("Email", "Invalid email.<br/><br/>");

                if (model.Remember == false)
                {
                    ModelState.AddModelError("Remember", "Please Check CheckBox");
                }

                return PartialView("~/ViewComponents/NewsletterSubscriptionWidget/_Subscribe.cshtml", model);
